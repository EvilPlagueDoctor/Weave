...
class DaemonClient(private val context: Context) {
    ...
    suspend fun connect(onStatus: (String) -> Unit) {
        ...
        try {
            ...
            // If this profile revoked/rotated the credential while Weave was away, the saved
            // secret is no longer useful. Remove only this profile's slot and perform the normal
            // approval flow once; credentials for other VeilKnit accounts remain untouched.
    ...
    fun daemonIdentityChanged(): Boolean {
        ...
        if (!state.optBoolean("ready", false)) return true
        val profile = state.optString("profile_id").trim()
        val instance = state.optString("daemon_instance_id").trim()
        return profile.isBlank() || instance.isBlank() ||
    ...
    private suspend fun awaitReadyDaemonIdentity(onStatus: (String) -> Unit): DaemonIdentity =
        withContext(Dispatchers.IO) {
            ...
            while (true) {
                ...
                val profile = state.optString("profile_id").trim()
                val instance = state.optString("daemon_instance_id").trim()
                if (state.optBoolean("ready", false) && profile.isNotBlank() && instance.isNotBlank()) {
                    return@withContext DaemonIdentity(profile, instance)
                }
                if (System.currentTimeMillis() >= deadline) {
                    val detail = state.optString("last_error").takeIf { it.isNotBlank() && it != "null" }
                        ?: state.optString("status").ifBlank { "daemon did not become ready" }
                    error("VeilKnit daemon was not ready after 60 seconds: $detail")
                }
                onStatus(state.optString("status").ifBlank { "Waiting for VeilKnit daemon…" })
                delay(500)
            }

            // `while (true)` is typed as Unit by Kotlin, even though every successful
            // path above returns a DaemonIdentity (and the timeout path throws).
            // Keeping a Nothing-typed expression as the lambda tail makes the
            // withContext return type unambiguous to the compiler.
            error("Unreachable: daemon readiness loop exited unexpectedly")
        }

    private fun clearCredential(profileId: String) {
                ...
                "app_registration_approved" -> {
                    credentialStore.write(
                        ...
                        SecureCredentialStore.Credential(
                            secretHex = status.getString("secret_hex"),
                            generation = status.getLong("credential_generation"),
                        )
                    )
                    return@withContext
                }
    ...
    private suspend fun authenticate(profileId: String) = withContext(Dispatchers.IO) {
        ...
        val savedCredential = credentialStore.read(profileId) ?: error("Missing app credential")
        val secret = savedCredential.secretHex.hexToBytes()
        val savedGeneration = savedCredential.generation
        if (generation != savedGeneration) {
            clearCredential(profileId)
            error("Credential generation changed; this VeilKnit profile must authorize Weave again")
        }
    ...
    fun recommendNodes(nodes: List<String>) = rawRequest("recommend_nodes") {
        put("nodes", JSONArray(nodes.take(256))); put("context", "weave_gossip"); put("ttl_seconds", 600)
    }
    ...
    // ---- Daemon-owned local encrypted app vault ------------------------------------------
    fun putPrivateValue(key: String, data: ByteArray, retention: String = "persistent", ttlSeconds: Long? = null): JSONObject =
        rawRequest("put_private_value") {
            put("key", key)
            put("value_base64", Base64.encodeToString(data, Base64.NO_WRAP))
            put("retention", retention)
            ttlSeconds?.let { put("ttl_seconds", it) }
        }.getJSONObject("value")

    fun getPrivateValue(key: String): ByteArray? {
        val result = rawRequest("get_private_value") { put("key", key) }
        return when (result.optString("type")) {
            "private_value_missing" -> null
            "private_value_read" -> Base64.decode(result.getString("value_base64"), Base64.DEFAULT)
            else -> error("Unexpected private-value response: $result")
        }
    }

    fun listPrivateValues(): List<JSONObject> {
        val array = rawRequest("list_private_values").optJSONArray("values") ?: JSONArray()
        return List(array.length()) { array.getJSONObject(it) }
    }

    fun deletePrivateValue(key: String): Boolean =
        rawRequest("delete_private_value") { put("key", key) }.optBoolean("deleted", false)

    fun renewPrivateValue(key: String, retention: String, ttlSeconds: Long? = null): JSONObject =
        rawRequest("renew_private_value") {
            put("key", key); put("retention", retention); ttlSeconds?.let { put("ttl_seconds", it) }
        }.getJSONObject("value")

    fun beginPrivateBlob(contentType: String, retention: String = "persistent", ttlSeconds: Long? = null): JSONObject =
        rawRequest("begin_private_blob") {
            put("content_type", contentType); put("retention", retention); ttlSeconds?.let { put("ttl_seconds", it) }
        }.getJSONObject("blob")

    fun appendPrivateBlob(blobId: String, data: ByteArray): JSONObject =
        rawRequest("append_private_blob") {
            put("blob_id", blobId); put("data_base64", Base64.encodeToString(data, Base64.NO_WRAP))
        }.getJSONObject("blob")

    fun finishPrivateBlob(blobId: String): JSONObject =
        rawRequest("finish_private_blob") { put("blob_id", blobId) }.getJSONObject("blob")

    fun abortPrivateBlob(blobId: String): Boolean =
        rawRequest("abort_private_blob") { put("blob_id", blobId) }.optBoolean("aborted", false)

    fun deletePrivateBlob(blobId: String): Boolean =
        rawRequest("delete_private_blob") { put("blob_id", blobId) }.optBoolean("deleted", false)

    fun renewPrivateBlob(blobId: String, retention: String, ttlSeconds: Long? = null): JSONObject =
        rawRequest("renew_private_blob") {
            put("blob_id", blobId); put("retention", retention); ttlSeconds?.let { put("ttl_seconds", it) }
        }.getJSONObject("blob")

    fun readPrivateBlobRange(blobId: String, offset: Long, length: Long): JSONObject =
        rawRequest("read_private_blob_range") { put("blob_id", blobId); put("offset", offset); put("length", length) }

    fun putPrivateBlob(contentType: String, data: ByteArray, retention: String = "persistent", ttlSeconds: Long? = null): JSONObject {
        val started = beginPrivateBlob(contentType, retention, ttlSeconds)
        val blobId = started.getString("blob_id")
        try {
            var offset = 0
            ...
            return finishPrivateBlob(blobId)
        } catch (t: Throwable) {
            runCatching { abortPrivateBlob(blobId) }
            throw t
        }
    }

    fun getPrivateBlob(blobId: String, maxBytes: Int = 32 * 1024 * 1024): ByteArray {
        val metadata = readPrivateBlobRange(blobId, 0, 0).getJSONObject("blob")
        val total = metadata.getLong("total_bytes")
        require(total in 0..maxBytes.toLong()) { "Private blob is too large for this operation: $total bytes" }
