...
object CommentChain {
    ...
    const val COMMENT_STORE_NAME = "weave_comments"
    const val COMMENT_CONTINUATION_NAME = "weave_comments_next"
    const val INDEX_STORE_NAME = "weave_comment_index"
    const val INDEX_CONTINUATION_NAME = "weave_comment_index_next"
...
data class IndexEntry(
    ...
    fun toBytes(): ByteArray = JSONObject()
        ...
        .put("author", authorKey)
        .put("id", commentId)
        .apply { if (digest.isNotBlank()) put("digest", digest) }
        .toString()
        .encodeToByteArray()
...
/** Owner-to-author acknowledgement for a comment notice. */
data class CommentAck(
    val commentId: String,
    val status: Status,
) {
    enum class Status { Published, WaitingApproval, Rejected }
    ...
    companion object {
        const val ACK_TYPE = "weave.comment.ack.v1"

        fun fromBytes(bytes: ByteArray): CommentAck? = runCatching {
            val json = JSONObject(bytes.decodeToString())
            if (json.optString("type") != ACK_TYPE) return null
            val id = json.optString("id")
            if (id.isBlank()) return null
            CommentAck(id, Status.valueOf(json.getString("status")))
        }.getOrNull()
    }
}

fun commentDigest(comment: WireComment): String =
    java.security.MessageDigest.getInstance("SHA-256")
        .digest(comment.toBytes())
