...
class CommentNetwork(private val client: DaemonClient) {
     ...
     * available.
     */
    fun post(comment: WireComment, ownerMainDht: String, notify: Boolean): CommentChain.Pointer {
        val root = ensureStores()
        val pointer = CommentChain.append(
            client, root, CommentChain.COMMENT_CONTINUATION_NAME, comment.toBytes()
        )
        if (notify && ownerMainDht.isNotBlank() && ownerMainDht != comment.authorKey) {
            runCatching {
                client.sendMessage(
                    recipientMainDht = ownerMainDht,
                    payload = CommentNotice(comment.pageKey, pointer, comment.id).toBytes(),
                )
            }
        }
        return pointer
