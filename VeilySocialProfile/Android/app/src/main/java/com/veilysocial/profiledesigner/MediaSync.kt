...
suspend fun publishPendingMedia(
    ...
    for (element in pending) {
        ...
        val file = media.fileFor(hash)
        if (!file.exists()) {
            return Result.failure(IllegalStateException("An image on this profile is missing from this device. Re-add it and publish again."))
        }
        val bytes = runCatching { file.readBytes() }.getOrNull()
            ?: return Result.failure(IllegalStateException("Could not read one of the images on this profile."))
        ...
        media.recordBlob(hash, blob.optString("blob_id"), recordKey)
        onProgress(++done, pending.size)
    }
...
class MediaLoader(
    ...
    private fun request(hash: String, recordKey: String) {
        ...
        scope.launch {
            try {
                permits.withPermit {
                    // Re-check after queueing: the person may have moved on while we waited.
                    ...
                    val bytes = controller.downloadMedia(
                        ...
                        keepGoing = { synchronized(inFlight) { wanted.isEmpty() || hash in wanted } },
                    )
                    if (bytes == null) {
                        synchronized(inFlight) { failed.add(hash) }
                        return@withPermit
                    }
                    media.storeVerifiedBytes(hash, bytes)
                    revision++
                }
            } finally {
